...
public class PlaygroundSolver
{
    ...
    public void PrintDistanceMatrix(double[][] matrix)
    {
        for (var i = 0; i < JunctionBoxes.Count; i++)
        {
            for (var j = 0; j < JunctionBoxes.Count; j++) Console.Write($"{matrix[i][j],8:F3} ");
    ...
    public int ConnectAll()
    {
        ...
        Console.WriteLine(
            $"Job is finished after {iterationCounter} iterations. Last Elements are {shorterDistance.FirstIterator}({JunctionBoxes[shorterDistance.FirstIterator]}) and {shorterDistance.SecondIterator}({JunctionBoxes[shorterDistance.SecondIterator]})");
        ...
        (int FirstIterator, int SecondIterator, double distance) minimalDistance)
    {
        ...
        //CASE 3 - both are
        if (circuitOfTheFirstElement is not null && circuitOfTheSecondElement is not null &&
            circuitOfTheFirstElement.index != circuitOfTheSecondElement.index)
        {
    ...
    {
        public double CalculateDistance(JunctionBox target)
        {
